...
func NewRabbitMQPublisher(url string) (*RabbitMQPublisher, error) {
	...
	publisher := &RabbitMQPublisher{conn: conn, channel: ch, url: url}
...
func (p *RabbitMQPublisher) DeclareQueue() error {
	q, err := p.channel.QueueDeclare(task.TaskQueueName, true, false, false, false, nil)
	if err != nil {
...
func (p *RabbitMQPublisher) Publish(taskID string, input string) error {
	message := task.TaskMessage{TaskID: taskID, Input: input}
...
func main() {
	...
	allowedOrigins := task.GetEnv("ALLOWED_ORIGINS", "http://localhost:3000,http://127.0.0.1:3000")
	...
	log.Printf("INFO: Allowed CORS Origins: %s", allowedOrigins)
...
func (app *Application) getTaskStatusHandler(c *gin.Context) {
	...
	selectSQL := `SELECT status, task_input, result, created_at, updated_at FROM tasks WHERE task_id = $1`
	var resp task.TaskStatusResponse
	resp.TaskID = taskIDParam
	err = app.DB.QueryRow(c.Request.Context(), selectSQL, taskIDParam).Scan(
		&resp.Status, &resp.Input, &resp.Result, &resp.CreatedAt, &resp.UpdatedAt,
	)
	if err != nil {
